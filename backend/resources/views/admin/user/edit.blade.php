...
<div class="max-w-xl bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <form action="{{ route('admin.user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
         ...
         <div class="mb-6">
                <label class="block text-gray-700 text-sm font-semibold mb-2">Foto Profile <span class="text-xs text-gray-400 font-normal">(Biarkan kosong jika tidak ingin mengubah foto profile)</span></label>
                @if($user->foto_profile)
                    <div class="mb-2">
                        <img src="{{ asset($user->foto_profile) }}" alt="Preview" class="w-16 h-16 object-cover rounded-lg border">
                    </div>
                @endif
