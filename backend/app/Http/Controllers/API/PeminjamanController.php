...
class PeminjamanController extends Controller
{
        ...
        try {
            $peminjaman = DB::transaction(function () use ($request) {
                ...
                foreach ($request->items as $item) {
                    // Lock baris data di database untuk menghindari race condition
    ...
    /**
     * Memperbarui permohonan peminjaman.
     */
    public function update(StorePeminjamanRequest $request, Peminjaman $peminjaman): JsonResponse
    {
        ...
        try {
            DB::transaction(function () use ($request, $peminjaman) {
                ...
                // Hapus item detail pinjam lama
                $peminjaman->detailPinjam()->delete();

                // Tambahkan item detail pinjam baru
                foreach ($request->items as $item) {
                    $alat = Alat::lockForUpdate()->findOrFail($item['alat_id']);
    ...
    /**
     * Membatalkan / menghapus peminjaman.
     */
    ...
    /**
     * Menyetujui permohonan peminjaman (khusus Petugas / Admin).
     */
    ...
    /**
     * Menampilkan riwayat peminjaman milik peminjam yang sedang login.
     */
